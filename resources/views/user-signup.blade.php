...
<body class="min-h-screen">
    ...
    <div class="bg-gray-100 flex justify-center items-center min-h-[calc(100vh-64px)] px-4 py-6 sm:py-8">
        ...
        <div class="bg-white p-6 sm:p-8 rounded-2xl shadow-lg w-full max-w-sm">
            ...
            <h2 class="text-xl sm:text-2xl text-center text-gray-800 mb-4 sm:mb-6 font-semibold">User Signup Page</h2>
            @error('user')
            ...
            <form action="/user-signup" method="post" class="space-y-4">
                ...
                <div>
                    ...
                    <label for="name" class="block text-sm sm:text-base text-gray-600 mb-1">User Name</label>
                    <!-- Improved input styling with better focus states -->
                    <input type="text" name="name" id="name" placeholder="Enter User name" value="{{ old('name') }}"
                ...
                <button type="submit"
                    class="w-full px-4 py-2.5 sm:py-3 text-sm sm:text-base text-white bg-blue-400 hover:bg-blue-500 rounded-lg text-center font-medium transition-colors cursor-pointer">Signup</button>
            </form>
        </div>
    </div>
</body>
